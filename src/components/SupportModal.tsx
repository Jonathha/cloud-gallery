...
export default function SupportModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(dbPrimary, 'support_messages'),
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.warn('SupportModal: Failed to subscribe to support messages.', error);
      setAuthError("Erro ao carregar mensagens de suporte.");
      setLoading(false);
    });
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(dbPrimary, 'support_messages'), {
      ...
      console.error("[Support Modal] Error sending message:", error);
    }
  };
